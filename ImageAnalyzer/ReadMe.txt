================================================================================
    MFC 程式庫：ImageAnalyzer 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 ImageAnalyzer 應用程式。這個應用程式不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案包含一份摘要，簡要說明構成 ImageAnalyzer 應用程式的
所有檔案，它們個別的內容。
...
ImageAnalyzer.h
    這是應用程式的主要標頭檔。
    它包含了特定標頭 (包括 Resource.h)，並宣告 CImageAnalyzerApp 應用程式類別。

ImageAnalyzer.cpp
    這是含有應用程式類別 CImageAnalyzerApp 的主要應用程式原始程式檔。

ImageAnalyzer.rc
    這是程式所用的所有 Microsoft Windows 資源的列表。它含有儲存在 RES 子目錄中的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 內編輯這個檔案。您的專案資源是在 1028。

res\ImageAnalyzer.ico
    這是用來做為應用程式圖示的圖示檔。這個圖示是包含在主要資源檔 ImageAnalyzer.rc 中。

res\ImageAnalyzer.rc2
...
ImageAnalyzerDoc.h、ImageAnalyzerDoc.cpp - 文件
    這些檔案包含您的 CImageAnalyzerDoc 類別。編輯這些檔案，可加入您的特殊文件類型並實作檔案儲存和載入 (透過 CImageAnalyzerDoc::Serialize)。

ImageAnalyzerView.h、ImageAnalyzerView.cpp - 文件的檢視表
    這些檔案包含您的 CImageAnalyzerView 類別。
    CImageAnalyzerView 物件是用來檢視 CImageAnalyzerDoc 物件。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 ImageAnalyzer.pch 的先行編譯標頭 (PCH) 檔，以及名為 StdAfx.obj 的先行編譯型別檔。

Resource.h
    這是標準標頭檔，它定義新的資源 ID。Microsoft Visual C++ 會讀取和更新這個檔案。

ImageAnalyzer.manifest
